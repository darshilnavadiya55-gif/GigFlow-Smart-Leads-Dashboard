    ...
    <div className={`min-h-screen flex items-center justify-center p-4 relative overflow-hidden theme-transition ${
      ...
      <div className={`rounded-2xl p-10 w-full max-w-md relative z-10 transition-all duration-500 theme-transition ${
        ...
        <div className="text-center mb-8">
          ...
          <h2 className={`text-3xl font-bold tracking-tight ${dark ? 'text-white' : 'text-gray-900'}`}>Create Account</h2>
          <p className={`mt-2 font-light ${dark ? 'text-slate-400' : 'text-gray-500'}`}>Join Smart Leads Dashboard today</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className={`block text-sm font-medium mb-2 ${dark ? 'text-slate-300' : 'text-gray-700'}`}>Email Address</label>
            <input
              type="email"
              value={email}
              ...
              } ${validationErrors.email ? 'border-red-500/50' : dark ? 'border-slate-700/50' : 'border-gray-200'}`}
              placeholder="name@company.com"
            />
            {validationErrors.email && <p className="text-red-400 text-xs mt-1.5">{validationErrors.email}</p>}
          </div>

          <div>
            <label className={`block text-sm font-medium mb-2 ${dark ? 'text-slate-300' : 'text-gray-700'}`}>Password</label>
            <input
              type="password"
              value={password}
              ...
              className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-500 transition-colors ${
                dark ? 'bg-slate-800/50 text-white' : 'bg-gray-50 text-gray-900'
              } ${validationErrors.password ? 'border-red-500/50' : dark ? 'border-slate-700/50' : 'border-gray-200'}`}
              placeholder="At least 8 characters"
            />
            {validationErrors.password && <p className="text-red-400 text-xs mt-1.5">{validationErrors.password}</p>}
          </div>

          <div>
            <label className={`block text-sm font-medium mb-2 ${dark ? 'text-slate-300' : 'text-gray-700'}`}>Confirm Password</label>
            <input
              type="password"
              ...
              className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-500 transition-colors ${
                dark ? 'bg-slate-800/50 text-white' : 'bg-gray-50 text-gray-900'
              } ${validationErrors.confirmPassword ? 'border-red-500/50' : dark ? 'border-slate-700/50' : 'border-gray-200'}`}
              placeholder="Confirm your password"
            />
            {validationErrors.confirmPassword && <p className="text-red-400 text-xs mt-1.5">{validationErrors.confirmPassword}</p>}
          </div>

          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium py-3 rounded-xl hover:from-blue-500 hover:to-purple-500 transition-all duration-300 shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:shadow-none flex justify-center items-center mt-4"
          >
            {isLoading ? (
